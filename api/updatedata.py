...
class JobTitleMatch:
    ...
    def __init__(self, jobtitles=[], kw=[], exclude_kw=[], sen=[], exclude_sen=[], jt=[], exclude_jt=[]):
        ...
        self.jobtitles_clean = [unidecode(re.sub(r"[^\w\s]", "", j)).lower(
        ).strip().replace("  ", " ") for j in jobtitles]
        self.kw_clean = [unidecode(re.sub(r"[^\w\s]", "", j)).lower(
        ).strip().replace("  ", " ") for j in kw]
        self.exclude_kw_clean = [unidecode(re.sub(r"[^\w\s]", "", j)).lower(
        ).strip().replace("  ", " ") for j in exclude_kw]
        self.sen_clean = [unidecode(re.sub(r"[^\w\s]", "", j)).lower(
        ).strip().replace("  ", " ") for j in sen]
        self.exclude_sen_clean = [unidecode(re.sub(r"[^\w\s]", "", j)).lower(
        ).strip().replace("  ", " ") for j in exclude_sen]
        self.jt_clean = [unidecode(re.sub(r"[^\w\s]", "", j)).lower(
        ).strip().replace("  ", " ") for j in jt]
        self.exclude_jt_clean = [unidecode(re.sub(r"[^\w\s]", "", j)).lower(
        ).strip().replace("  ", " ") for j in exclude_jt]
    ...
    def add_extra_titles(self):
        ...
        extra_sen = [
            ['manager', 'mgr'],
            ['svp', 'senior vice president', 'sr vp', 'sr vice president',
                'snr vp', 'snr vice president', 'senior vp'],
            ['vp', 'vice president']
        ]
        ...
        def check(lst, include, exclude):
            ...
            for i in lst:
                for j in i:
                    if j in include:
                        include.extend(
                            [k for k in i if k not in include and k not in exclude])
                        break
        ...
        self.kw_clean.extend(
            [f'{i}s' for i in self.kw_clean if f'{i}s' not in self.kw_clean and i[-1] != 's'])
        self.kw_clean.extend(
            [i[:-1] for i in self.kw_clean if i[:-1] not in self.kw_clean and i[-1] == 's'])
        ...
        self.exclude_kw_clean.extend(
            [f'{i}s' for i in self.exclude_kw_clean if f'{i}s' not in self.exclude_kw_clean and i[-1] != 's'])
        self.exclude_kw_clean.extend(
            [i[:-1] for i in self.exclude_kw_clean if i[:-1] not in self.exclude_kw_clean and i[-1] == 's'])
            ...
            for index1, i in enumerate(self.exclude_clean):
                ...
                if re.search(rf'\b{temp}\b', comp) or re.search(rf'\b{comp}\b', temp):
                    ...
                    if ':' in i:
                        ...
                        matched_companies[self.companies[index]
                                          ] = self.exclude[index1]
                    break
